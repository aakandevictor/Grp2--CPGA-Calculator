...
class Student(Person):
    ...
    def compute_cgpa(self) -> float:
        ...
        self.__gpa = round(total_points / total_units, 2)
        return self.__gpa

    def generate_transcript(self) -> str:
        semester_lines = "\n\n".join(
            semester.generate_transcript_section() for semester in self.__semesters
        )
        return (
            f"Transcript for {self.full_name} ({self.matric_no})\n"
            f"Department: {self.department}\n"
            f"Level: {self.level}\n"
            f"CGPA: {self.__gpa:.2f}\n"
            f"{semester_lines}"
        )


class Lecturer(Person):
    """Concrete lecturer class representing a university faculty member."""

    _VALID_RANKS = ["LECTURER", "SR_LECTURER", "ASSOC_PROF", "PROFESSOR"]
    _STAFF_PATTERN = re.compile(r"^[A-Z]{3,6}/STAFF/\d{4}$")
    ...
    def staff_id(self, value: str) -> None:
        if not isinstance(value, str) or not self._STAFF_PATTERN.match(value):
            raise ValueError("staff_id must match pattern [A-Z]{3,6}/STAFF/YYYY")
    ...
    def rank(self, value: str) -> None:
        if not isinstance(value, str) or value not in self._VALID_RANKS:
            raise ValueError(
                f"rank must be one of {self._VALID_RANKS}, got {value}"
            )
        self._rank = value

    def __str__(self) -> str:
        return (
            f"Lecturer: {self.full_name} | ID: {self.staff_id} | Rank: {self.rank} | "
